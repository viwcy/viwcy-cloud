...
public class RocketMQProducerConfiguration {
    ...
    public DefaultMQProducer getRocketMQProducer() {
        ...
        try {
            producer.start();
            log.info("RocketMQ-Producer initialize successful，groupName = [{}]，namesrvAddr = [{}]", this.groupName, this.namesrvAddr);
        } catch (MQClientException e) {
            e.printStackTrace();
            log.error("RocketMQ-producer initialize failed，reason = [{}]", e.getCause());
        }
        return producer;
    }
